...
<div class="panel-header">
    ...
    <ul class="nav nav-tabs text-regular" role="tablist">
        <li role="presentation" class="active">
            <a href="{{ Request::is('admin/inversion-agro/create') ? '/#perfil' : '#perfil' }} " aria-controls="perfil" role="tab" data-toggle="tab">Entradas</a>
        </li>
        <li role="presentation">
            <a href="{{ Request::is('admin/inversion-agro/create') ? '/#solicitud' : '#solicitud' }}" aria-controls="solicitud" role="tab" data-toggle="tab">Crear Nuevo</a>
        </li>
    </ul>
</div>
<div>
	...
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane active" id="perfil"><br>
			...
			@if(isset($noticias))
				@foreach($noticias as $directorio)
					<tr>
						<td>
							<h5 class="text-center">
								{{$directorio['id']}}
						...
						<td>
							<div class="small text-capitalize" style="max-width: 260px;">
								{!! substr($directorio['descripcion'], 0, 100)!!}...
							</div>
						</td>
						<td>
							<h5 class="text-center">
								@if($directorio['imagen'] != NULL)
									<img src="{{asset($directorio['imagen'])}}" alt="" class="img-responsive center-block" style="height: 50px;">
								@else
									No hay imagen
								@endif
							</h5>
						...
						<td class="text-center">
							<a href="{{url('admin/noticias/'.$directorio['id'])}}" class="text-center text-info btn">
		...
		<div role="tabpanel" class="tab-pane " id="solicitud"><br>
			<h3 class="text-bold text-success">Creación de Item</h3>
				...
				{!! Form::open(array('url'=> '/admin/noticias','method'=> 'POST', 'class' => '', 'files' => true , )) !!}
			    {!! Form::token() !!}
...
@if(isset($noticias))
	@foreach($noticias as $directorio)
		...
		<div class="modal fade" id="myModal{{$directorio['id']}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		    ...
		    {!! Form::open(array('url'=> '/admin/noticias/'.$directorio['id'], 'method'=> 'DELETE', 'files' => true ,)) !!}
		    {!! Form::token() !!}
