...
<div class="panel-header">
    ...
    <ul class="nav nav-tabs text-regular" role="tablist">
        <li role="presentation" class="active">
            <a href="{{ Request::is('administrador/productos/create') ? '/#perfil' : '#perfil' }} " aria-controls="perfil" role="tab" data-toggle="tab">Editar producto</a>
        </li>
        <li role="presentation">
            <a href="{{ Request::is('administrador/productos/create') ? '/#solicitud' : '#solicitud' }}" aria-controls="solicitud" role="tab" data-toggle="tab">Más Productos</a>
        </li>
    </ul>
</div>
<!-- Tab panes -->
...
@if(isset($productos))
	<h3 class="text-regular"><strong>Editar Item:</strong> {{$productos['titulo']}} </h3>
    {!! Form::open(array('url'=> '/administrador/productos/'.$productos['id'], 'method'=> 'PUT', 'files' => true ,)) !!}
    {!! Form::token() !!}
		<div class="form-horizontal">
    		<div class="form-group">
    			<label for="" class="control-label col-md-2">Nombre:</label>
    			<div class="col-md-6">
    				<input type="text" class="form-control" value="{{$productos['titulo']}}" name="titulo" placeholder="Nombre">
    			</div>
    		</div>
    		<div class="form-group">
    			<label for="" class="control-label col-md-2">Archivo:</label>
    			<div class="col-md-6">
    				<input type="file" class="form-control" value="{{$productos['archivo']}}" name="archivo" placeholder="Archivo">
    			</div>
    		</div>
    		<div class="form-group">
    			<label for="" class="control-label col-md-2">Descripción:</label>
    			<div class="col-md-6">
    				<textarea name="descripcion" class="form-control" placeholder="Descripción de Sider..." rows="7">{{$productos['informacion']}}</textarea>
    			</div>
    		</div>
    		<div class="form-group">
    			<div class="col-md-offset-3 col-md-2">
    				<input type="submit" class="btn btn-success" value="Actualizar productos">
    			</div>
    			<div class="col-md-3">
    				<a href="{{url('administrador/productos')}}" class="btn btn-danger">Cancelar y Regresar</a>
    			</div>
    		</div>
    	</div>
	{{Form::close()}}
@endif
        ...
        <div role="tabpanel" class="tab-pane" id="solicitud"><br>
            <span class="text-regular"><strong></strong> {{$productos['titulo']}} </span>
            ...
            <div><br>
                <table class="table table">
                    ...
                    @if(isset($productosmas))
                        @foreach($productosmas as $serviciosma)
                            <tr>
            ...
            <div class="modal fade" id="myModala" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                ...
                {!! Form::open(array('url'=> '/administrador/productosmas', 'method'=> 'POST', 'files' => true ,)) !!}
                {!! Form::token() !!}
                            ...
                            <div class="form-horizontal"><br>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-10">
                                        ...
                                        <input type="hidden" class="form-control" value="{{$productos['id']}}" name="producto_id" placeholder="Nombre">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="" class="control-label col-md-2">Archivo:</label>
...
@if(isset($productosmas))
    ...
    <div class="modal fade" id="myModale{{$serviciosma['id']}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            {!! Form::open(array('url'=> '/administrador/productosmas/'.$serviciosma['id'], 'method'=> 'PUT', 'files' => true ,)) !!}
            {!! Form::token() !!}
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
@if(isset($productosmas))
    @foreach($productosmas as $serviciosma)
        ...
        <div class="modal fade" id="myModal{{$serviciosma['id']}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
            {!! Form::open(array('url'=> '/administrador/productosmas/'.$serviciosma['id'], 'method'=> 'DELETE', 'files' => true ,)) !!}
            {!! Form::token() !!}
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
